...
public class TUserServiceImpl extends ServiceImpl<TUserMapper, TUser> implements ITUserService {
    ...
    public R loginUser(TUser user) {
        ...
        TUser one = getOne(new LambdaQueryWrapper<TUser>().eq(TUser::getUserName, user.getUserName()).eq(TUser::getUserCode, user.getUserCode()));

        return one == null? R.failed("密码错误！") : R.ok(jwtUtil.setToken(one)).setCode(1).setMsg("登录成功");
    }
    ...
    public R addUser(TUser user) {
        ...
        if (one != null)return R.failed("用户已存在！");
    ...
    @Override
    public R updateUser(TUser user) {
